...
def _prepare_email_addresses(
    ...
    """
    if not _validate_email_address(from_email):
...
def send_email(
    ...
    Args:
        ...
        region_name: AWS region for SES client
    ...
    """
    try:
        _from_email, _to_email, _cc_email, _bcc_email = _prepare_email_addresses(
        ...
        logger.debug(f'Email body: {body_html[:100]}...')  # Log only first 100 chars
        
        response = ses_client(region_name).send_email(
        ...
        logger.info(f"Email sent successfully. MessageId: {response.get('MessageId')}")
        return response
