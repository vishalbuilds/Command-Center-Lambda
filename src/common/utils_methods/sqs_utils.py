...
def _create_message_attributes(message_attr: dict) -> dict:
    """
...
def send_message(queue: str, message: str, region: str, message_attr: dict = None):
    """
    ...
    """
    try:
        message_attributes = _create_message_attributes(message_attr)
        logger.info(f'Sending message to {queue} with body: {message} and attributes: {message_attributes}')
        
        response = sqs_client(region).send_message(
            QueueUrl=queue,
            MessageBody=message,
...
def receive_message(
    ...
    try:
        ...
        while polling_attempt < max_polling_attempts:
            response = sqs_client(region_name).receive_message(
                QueueUrl=queue,
                MessageAttributeNames=list(message_ids.keys()) if message_ids else ['All'],
            ...
            if 'Messages' not in response:
                logger.info(f"No messages found in queue (attempt {polling_attempt + 1}/{max_polling_attempts})")
                polling_attempt += 1
                continue
            ...
            for message in response['Messages']:
                ...
                if receipt_handle in checked_receipt_handles:
                    logger.debug(f"Skipping already-checked message with receipt handle: {receipt_handle}")
                    continue
                ...
                if all(
                    key in message_attr and message_attr[key].get('StringValue') == value 
                    for key, value in message_ids.items()
                ):
                    logger.info(f"Found matching message for attributes {message_ids}")
                    
                    if auto_delete:
                        delete_message(receipt_handle, queue, region_name)
                        logger.info(f"Message with attributes {message_ids} auto-deleted from queue")
                    
                    return message
                else: 
                    # Return non-matching message to queue immediately
                    change_message_visibility(queue, receipt_handle, region_name, visibility_timeout=0)
                    logger.debug(f"Returned non-matching message to queue")
            
            polling_attempt += 1
        
        logger.warning(f"Message with attributes {message_ids} not found after {max_polling_attempts} attempts")
        logger.info(f"Total unique messages checked: {len(checked_receipt_handles)}")
        return None
...
def change_message_visibility(
    ...
    """
    try:
        sqs_client(region_name).change_message_visibility(
            QueueUrl=queue,
...
def delete_message(receipt_handle: str, queue_url: str, region_name: str):
    """
    ...
    """
    try:
        sqs_client(region_name).delete_message(
            QueueUrl=queue_url,
            ReceiptHandle=receipt_handle
        )
        logger.info(f"Message deleted successfully with receipt_handle: {receipt_handle}")
