...
class TestLambdaHandler:
    ...
    def sample_event(self):
        ...
        return {"request_type": "StatusCheckerConnect", "data": {"test": "value"}}
    ...
    def test_lambda_handler_success(
        ...
        mock_event_sanitizer,
        mock_extract_event_data,
        mock_get_invocation_source,
        mock_logger,
        mock_context,
        sample_event,
    ):
    ...
    @patch("lambda_handler.Logger")
    def test_lambda_handler_trace_id_error(
        self, mock_logger, mock_context, sample_event
    ):
        """Test lambda handler with trace ID initialization error."""
        # Simulate error in get_invocation_source
        with patch(
            "lambda_handler.get_invocation_source",
            side_effect=Exception("Trace ID error"),
        ):
            result = lambda_handler(sample_event, mock_context)
        assert isinstance(result, dict)
    ...
    @patch("lambda_handler.Logger")
    @patch("lambda_handler.get_invocation_source")
    def test_lambda_handler_invocation_source_error(
        self, mock_get_invocation_source, mock_logger, mock_context, sample_event
    ):
    ...
    @patch("lambda_handler.extract_event_data")
    @patch("lambda_handler.EventSanitizer")
    def test_lambda_handler_sanitizer_error(
        self,
        mock_event_sanitizer,
        mock_extract_event_data,
        mock_get_invocation_source,
