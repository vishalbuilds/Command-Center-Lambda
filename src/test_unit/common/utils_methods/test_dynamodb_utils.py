...
from src.common.utils_methods.dynamodb_utils_resource import (
...
class TestDynamoDBUtils:
    ...
    def test_query_items_by_key_eq_success(
        self, mock_condition_expr, mock_dynamodb_table
    ):
        ...
        result = query_items_by_key_eq(
            "test-table", "status-index", "status", "active", "us-east-1"
        )

        assert len(result) == 2
        assert result[0]["status"] == "active"
        mock_table.query.assert_called_once_with(
            IndexName="status-index", KeyConditionExpression=mock_condition
        )
    ...
    def test_build_dynamodb_update_expression_single_field(self):
        ...
        update_expression, attr_names, attr_values = _buid_dynamoDB_update_expression(
            update_data
        )
    ...
    def test_update_single_item_by_pk_success(self, mock_dynamodb_table):
        ...
        update_single_item_by_pk(
            "test-table", update_data, "id", "test-id", "us-east-1"
        )
