...
class AutoCleanUpActiveContacts(DefaultStrategy):
    def __init__(self, event):
        ...
        LOGGER.info(f"Initializing AutoCleanUpActiveContacts for instance: {self.instance_id}")

    def do_validate(self):
        LOGGER.info("Starting validation process")
        
        if not self.instance_id:
            LOGGER.error("Validation failed: INSTANCE_ID environment variable is not set")
            return False
        ...
        LOGGER.info("Validation successful: All required environment variables are present")
        return True
    ...
    def _routing_profile_arn(self):
        ...
        try:
            ...
            LOGGER.info(f"Successfully retrieved {len(routing_profile_arns)} routing profile ARNs")
            ...
            LOGGER.error(f"Failed to retrieve routing profile ARNs for instance {self.instance_id}: {str(e)}")
            raise
    ...
    def _active_contact_ids(self, routing_profile_arn: list):
        ...
        """ 
        LOGGER.info(f"Fetching active contacts for {len(routing_profile_arn)} routing profiles")
        ...
        try:
            ...
            for user in user_data_list:
                for contact in user.get('Contacts', []):
                    ...
                    if not ts:
                        LOGGER.warning(f"Contact {contact.get('ContactId')} missing ConnectedToAgentTimestamp")
                        continue

                    connected_to_agent_timestamp = datetime.fromisoformat(ts) if isinstance(ts, str) else ts
                    duration_hours = (datetime.now(timezone.utc) - connected_to_agent_timestamp).total_seconds() / 3600

                    if contact.get('AgentContactState') == 'CONNECTED' and duration_hours >= MAX_CONTACT_ACTIVE_TIME:
                        contact_id = contact.get('ContactId')
                        active_contact_ids_list.append(contact_id)
                        contacts_exceeding_threshold += 1
                        LOGGER.info(f"Contact {contact_id} exceeds threshold: {duration_hours:.2f} hours active")
            
            LOGGER.add_tempdata("contacts_checked", contacts_checked)
            LOGGER.add_tempdata("contacts_exceeding_threshold", contacts_exceeding_threshold)
            LOGGER.add_tempdata("active_contact_ids", active_contact_ids_list)
            LOGGER.info(f"Found {len(active_contact_ids_list)} contacts exceeding {MAX_CONTACT_ACTIVE_TIME} hour threshold out of {contacts_checked} checked")
    ...
    def _process_contact_validation_and_disconnect(self, contact_id):
        ...
        try:
            ...
            if contact.get('DisconnectTimestamp'):
                ...
                LOGGER.info(f"Contact {contact_id} is already disconnected at {disconnect_ts}")
                return {
            ...
            last_update_timestamp = datetime.fromisoformat(ts) if isinstance(ts, str) else ts
            duration_hours = (datetime.now(timezone.utc) - last_update_timestamp).total_seconds() / 3600
            
            LOGGER.add_tempdata("contact_duration_hours", f"{duration_hours:.2f}")
            
            if duration_hours >= MAX_CONTACT_ACTIVE_TIME:
                LOGGER.info(f"Attempting to disconnect contact {contact_id} (active for {duration_hours:.2f} hours)")
                ...
                LOGGER.info(f"Contact {contact_id} not disconnected: active for {duration_hours:.2f} hours (threshold: {MAX_CONTACT_ACTIVE_TIME} hours)")
    ...
    def do_operation(self):
        ...
        """
        LOGGER.info(f"Starting contact cleanup operation with {MAX_CONTACT_ACTIVE_TIME} hour threshold")
        ...
        try:
            ...
            LOGGER.info(f"Processing {len(rp_arn_list)} routing profiles in batches of {RP_ARN_LIMITS}")
            
            # Get active contacts in batches
            all_active_contacts = []
            for i in range(0, len(rp_arn_list), RP_ARN_LIMITS):
                batch_arns = rp_arn_list[i:i + RP_ARN_LIMITS]
                LOGGER.info(f"Processing batch {i//RP_ARN_LIMITS + 1}: {len(batch_arns)} routing profiles")
            ...
            for idx, contact_id in enumerate(all_active_contacts, 1):
                LOGGER.info(f"Processing contact {idx}/{len(all_active_contacts)}: {contact_id}")
                
                try:
                    contact_disconnect_status = self._process_contact_validation_and_disconnect(contact_id)
                    ...
                    if status == 'Disconnected':
                        ...
                        contact_result.append({
                            ...
                            'LastUpdateTimestamp': str(contact_disconnect_status.get('LastUpdateTimestamp')),
                            'contact_id': contact_id,
                            'duration_hours': contact_disconnect_status.get('duration_hours')
                        })
                        LOGGER.add_tempdata("disconnected_contact_id", contact_id)
                        ...
                        LOGGER.add_tempdata("in_progress_contact_id", contact_id)
                        LOGGER.info(f"Contact {contact_id} still in progress (below threshold)")
                    ...
                    LOGGER.error(f"Failed to process contact {contact_id}: {str(contact_error)}")
